 ...
 public class BinaryTree {
      ...
      */
      ...
      private void replaceValueHelper(Node node, int oldVal, int newVal) {
        // If the node is null, there's nothing to update.
        if (node == null) {
            return;
        }
        ...
        // After processing children, update the current node if needed.
        if (node.data == oldVal) {
            node.data = newVal;
        }
    }
      ...
      */
      ...
      private int findMinHelper(Node node) {
        ...
        // Determine the smallest value using conditional checks.
        if (node.data <= minLeft && node.data <= minRight) {
            return node.data;
      ...
      */
      ...
      private int nodesGTHelper(Node node, int val) {
        // If there's no node, return 0 since there are no values to compare.
        if (node == null) {
            return 0;
        }
        
        // First, count the nodes in the left and right subtrees.
        int count = nodesGTHelper(node.left, val) + nodesGTHelper(node.right, val);
        
        // Then, if the current node's data is greater than 'val', add 1 to the total.
        if (node.data > val) {
            count++;
        }
        
        // Return the cumulative count for this branch of the tree.
        return count;
    }
     ...
     private int[] averageHelper(Node n) {
        // Base condition: if the node is null, return an array with sum 0 and count 0.
        if (n == null) {
            return new int[]{0, 0};
        }
        
        // Recursively retrieve sum and count from the left and right subtrees.
        int[] left = averageHelper(n.left);
        int[] right = averageHelper(n.right);
        
        // Calculate the current total sum and count including this node.
        int totalSum = n.data + left[0] + right[0];
        int totalCount = 1 + left[1] + right[1];
        
        // Return an array where index 0 holds the total sum and index 1 holds the node count.
